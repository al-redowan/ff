...
import { GroupIcon, CheckCircleIcon, XCircleIcon, WandIcon, ListBulletIcon, DocumentArrowUpIcon, UserPlusIcon } from './constants';
...
const App: React.FC = () => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (file.type === 'text/csv' || file.name.endsWith('.csv')) {
            ...
            newTeams = dataLines
              ...
              .map((line, index) => {
                ...
                return { id: Date.now() + index, name, leaderWhatsapp, email };
            });
          ...
          setTeams(currentTeams => {
            const existingNames = new Set(currentTeams.map(t => t.name.toLowerCase()));
            const uniqueNewTeams = newTeams.filter(nt => !existingNames.has(nt.name.toLowerCase()));
            return [...currentTeams, ...uniqueNewTeams];
          });
  ...
  const handleAddTeam = useCallback((newTeamData: Omit<Team, 'id'>) => {
    ...
    setTeams(prevTeams => {
        const existingNames = new Set(prevTeams.map(t => t.name.toLowerCase()));
        if (existingNames.has(newTeam.name.toLowerCase())) {
            // Handle duplicate name case if necessary, e.g., show a warning
            return prevTeams;
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 space-y-4 flex flex-col">
                  ...
                  <button
                      ...
                      disabled={isAnalyzing || teams.length === 0}
                      className="w-full mt-auto flex items-center justify-center space-x-2 bg-indigo-600 text-white px-4 py-2.5 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-indigo-500 disabled:bg-indigo-800 disabled:text-slate-400 disabled:cursor-not-allowed transition-colors"
                  >
                ...
                {/* Group Generation Card */}
                 <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 space-y-4 flex flex-col">
                    ...
                    <button
                        ...
                        disabled={teams.length === 0 && quickGroupNames.trim() === ''}
                        className="w-full mt-auto flex items-center justify-center space-x-2 bg-indigo-600 text-white px-4 py-2.5 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-indigo-500 disabled:bg-indigo-800 disabled:text-slate-400 disabled:cursor-not-allowed transition-colors"
                    >
